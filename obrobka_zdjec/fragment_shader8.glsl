...
uniform float width;
uniform float value;
//http://coding-experiments.blogspot.ch/2011/01/toon-pixel-shader.html
...
vec4 get_pixel(vec2 coords, float dx, float dy) {
 return texture(myTextureSampler,coords + vec2(dx, dy));
}

// returns pixel color
float IsEdge(in vec2 coords){
  float dxtex = 1.0 /float(textureSize(myTextureSampler,0)) ;
  float dytex = 1.0 /float(textureSize(myTextureSampler,0));
  float pix[9];
  int k = -1;
  float delta;
...
void main(void)
{
    ...
    vec4 colorOrg = texture( myTextureSampler, coord );
    vec3 vHSV =  RGBtoHSV(colorOrg.r,colorOrg.g,colorOrg.b);
    vHSV.x = nearestLevel(vHSV.x, 0);
    vHSV.y = nearestLevel(vHSV.y, 1);
    vHSV.z = nearestLevel(vHSV.z, 2);
    float edg = IsEdge(coord);
    vec3 vRGB = (edg >= value)? vec3(0.0,0.0,0.0):HSVtoRGB(vHSV.x,vHSV.y,vHSV.z);
    pixelColors = vec4(vRGB.x,vRGB.y,vRGB.z,1.0);
}
